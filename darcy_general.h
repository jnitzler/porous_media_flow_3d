...
template <int dim>
void Darcy<dim>::generate_ref_input() {
...
template <int dim>
void Darcy<dim>::generate_coordinates() {
...
// ------ read input file ------------------------------
template <int dim>
void Darcy<dim>::read_input_npy(const std::string &filename) {
...
template <int dim>
void Darcy<dim>::assemble_preconditioner() {
  ...
  FEValues<dim> fe_rf_values(rf_fe_system, quadrature_formula,
...
// ------------- assemble system -----------------
template <int dim>
void Darcy<dim>::assemble_system() {
  ...
  for (const auto &cell_tria : triangulation.active_cell_iterators()) {
    ...
    if (cell->is_locally_owned()) {
      ...
      // get rf function values and permeability tensor per cell
      // note we assume the same quadrature points for random field and solution
      // values
      std::vector<double> rf_values(n_q_points);
      Tensor<2, dim> K_mat;
      fe_rf_values.get_function_values(x_vec, rf_values);
      ...
      for (unsigned int q = 0; q < n_q_points; ++q) {
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i) {
          ...
          for (unsigned int j = 0; j <= i; ++j) {
            ...
            local_matrix(i, j) +=
                (phi_u[i] * k_inverse * phi_u[j] - phi_p[i] * div_phi_u[j] -
                 div_phi_u[i] * phi_p[j]) *
                fe_values.JxW(q);
          }  // end inner dof loop
...
// ---------- setup system matrix ------------------------
template <int dim>
void Darcy<dim>::setup_system_matrix(
  ...
  TrilinosWrappers::BlockSparsityPattern sp(
      partitioning, partitioning, relevant_partitioning, MPI_COMM_WORLD);
...
template <int dim>
void Darcy<dim>::setup_preconditioner(
    const std::vector<IndexSet> &partitioning,
    const std::vector<IndexSet> &relevant_partitioning) {
  ...
  DoFTools::make_sparsity_pattern(
      dof_handler, coupling_precond, sp, constraints, false,
      Utilities::MPI::this_mpi_process(MPI_COMM_WORLD));
  sp.compress();
...
// --------- setup grid and dofs ----------------
template <int dim>
void Darcy<dim>::setup_grid_and_dofs() {
  ...
  solution_distributed.reinit(partitioning, relevant_partitioning,
                              MPI_COMM_WORLD);
  solution_primary_distributed.reinit(partitioning, relevant_partitioning,
                                      MPI_COMM_WORLD);
}

// ------------- solver ----------------------
template <int dim>
void Darcy<dim>::solve() {
  ...
  for (unsigned int i = 0; i < solution.size(); ++i)
    if (constraints.is_constrained(i)) solution(i) = 0;
  ...
  for (unsigned int i = start; i < end; ++i)
    if (constraints.is_constrained(i)) distributed_solution(i) = 0;

  pcout << "Starting iterative solver..." << std::endl;
  solver_system.solve(system_matrix, distributed_solution, system_rhs,
...
template <int dim>
void Darcy<dim>::write_data_to_npy(const std::string &filename,
                                   std::vector<double> &data,
                                   const unsigned int rows,
                                   const unsigned int columns) const {
  const std::vector<long unsigned> shape{rows, columns};
  const bool fortran_order{false};
  npy::SaveArrayAsNumpy(filename, fortran_order, shape.size(), shape.data(),
                        data);
}

...
template <int dim>
void Darcy<dim>::output_velocity_at_observation_points_npy(
  ...
  const auto data_array = VectorTools::point_values<dim>(
...
void Darcy<dim>::output_field_at_observation_points_npy(
  ...
  VectorTools::interpolate_to_different_mesh(rf_dof_handler, x_vec,
                                             coarse_dof_handler, coarse_x_vec);

  // Step 3: Transfer back to the fine mesh
  Vector<double> x_smoothed(x_vec.size());
  VectorTools::interpolate_to_different_mesh(coarse_dof_handler, coarse_x_vec,
                                             rf_dof_handler, x_smoothed);
...
// ----------------- output_npy -----------------
template <int dim>
void Darcy<dim>::output_full_velocity_npy(const std::string &output_path) {
...
// ----------------- output pvtu -----------------
template <int dim>
void Darcy<dim>::output_pvtu(const std::string &output_path) const {
  ...
  data_out.write_vtu_with_pvtu_record(stripped_path, filename, cycle,
                                      MPI_COMM_WORLD, n_digits_counter,
                                      num_vtu_files);
  ...
  data_out_rf.build_patches(mapping, degree + 1,
                            DataOut<dim>::curved_inner_cells);
  data_out_rf.write_vtu_with_pvtu_record(stripped_path_rf, filename_rf, cycle,
                                         MPI_COMM_WORLD, n_digits_counter,
                                         num_vtu_files);
}

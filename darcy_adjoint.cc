...
template <int dim>
void Darcy<dim>::read_upstream_gradient_npy(
  ...
  const std::string adjoint_file_path = base_dir.string() + "/adjoint_data.npy";
  pcout << "Reading adjoint data from: " << adjoint_file_path << std::endl;

  std::vector<unsigned long> shape{};
  bool fortran_order;

  npy::LoadArrayFromNumpy(adjoint_file_path, shape, fortran_order,
                          adjoint_data_vec);
...
template <int dim>
void Darcy<dim>::read_primary_solution(const std::string &output_path) {
  TimerOutput::Scope timing_section(computing_timer,
                                    "read primary solution npy");
  std::vector<unsigned long> shape{};
  bool fortran_order;
  ...
  if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0) {
    npy::LoadArrayFromNumpy(filename, shape, fortran_order,
...
template <int dim>
void Darcy<dim>::overwrite_adjoint_rhs() {
    ...
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i) {
        ...
        for (unsigned int k = 0; k < data_element_idx.size(); ++k) {
          ...
          // get shape function value at current point for current dof-shape fun
          double shape_value = fe.shape_value(i, current_cell_point);

          // small hack to filter the correct component --> for specific dof all
          // but one component should be 0
          ...
          for (unsigned int comp = 0; comp < components.size(); ++comp) {
            if (components[comp]) {
              grad_log_lik =
                  data_vec[idx][comp];  // simple double for grad_log_lik value
                                        // of interest
              break;  // Assuming only one component is non-zero per shape
                      // function
            }         // end if statement component
          }           // end loop components

          // write into global rhs (both components for velocity)
          system_rhs(local_dof_indices[i]) += -grad_log_lik * shape_value;
...
// ---------------------- run method adjoint --------------------------------
template <int dim>
void Darcy<dim>::run(const std::string &input_path,
...
template <int dim>
void Darcy<dim>::run_simulation(const std::string &input_path,
                                const std::string &output_path) {
  setup_grid_and_dofs();
  read_input_npy(input_path);
  // generate_ref_input();
  read_primary_solution(output_path);  // this needs the dof handler hence after
                                       // setup_grid_and_dofs
...
template <int dim>
void Darcy<dim>::final_inner_adjoint_product() {
  ...
  FEValues<dim> fe_rf_values(rf_fe_system, quadrature,
  ...
  for (const auto &cell_tria : triangulation.active_cell_iterators()) {
    ...
    const auto &rf_cell = cell_tria->as_dof_handler_iterator(rf_dof_handler);
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i) {
          ...
          for (unsigned int j = 0; j < dofs_per_cell; ++j) {
            ...
            for (unsigned int k = 0; k < rf_dofs_per_cell; ++k) {
              RandomMedium::get_jacobi_inv_kmat(rf_value[q],
              ...
              grad_log_lik_x_distributed[local_rf_dof_indices[k]] +=
                  local_solution_i *
                  (phi_i_u * jacobi_k_mat_inv_value * phi_j_u * JxW_q) *
                  local_primary_j;
  ...
  // gradient
  Utilities::MPI::sum(grad_log_lik_x_distributed, MPI_COMM_WORLD,
                      grad_log_lik_x);
